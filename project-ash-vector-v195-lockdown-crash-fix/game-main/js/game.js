(() => {
  ...
  const BUILD_VERSION = '1.0.05';
  const BUILD_TITLE = 'LOCKDOWN STABILITY FIX';
  const bootLines = [
...
,
  ...
  // v195: lockdown enemies/projectiles use lazy image loading. The V194 build
  // referenced ensureImageCached during render but did not define it, which could
  // break the game the moment a random event tried to draw monsters/projectiles.
  function ensureImageCached(path){
    if(!path || typeof path !== 'string') return null;
    if(images[path]) return images[path];
    try{
      const im = new Image();
      im.onload = () => { try{ render(); }catch(err){} };
      im.onerror = () => console.warn('Missing lockdown asset:', path);
      im.src = path;
      images[path] = im;
      return im;
    }catch(err){
      console.warn('Could not cache lockdown asset:', path, err);
      return null;
    }
  }
  ...
  function loadImages(){
    const paths = [
      ...
      'assets/projectiles/ash_bolt.png','assets/projectiles/rust_spike.png','assets/projectiles/vector_pulse.png','assets/projectiles/bone_dart.png','assets/projectiles/toxic_splinter.png','assets/projectiles/scrap_shot.png','assets/projectiles/ember_round.png','assets/projectiles/static_needle.png','assets/projectiles/corrupted_shard.png','assets/projectiles/plasma_cube.png','assets/projectiles/blood_spark.png','assets/projectiles/ash_disc.png','assets/projectiles/echo_pellet.png','assets/projectiles/grave_flame.png'
    ];
  ...
  function startVectorLockdown(warning=null){
    ...
    window._avLockdownTimer=setInterval(()=>{
      try{ tickVectorLockdown(); }
      catch(err){ handleVectorLockdownRuntimeError(err); }
    },60);
    renderAll();
  }
  ...
  function handleVectorLockdownRuntimeError(err){
    console.error('Vector Lockdown runtime error:', err);
    if(window._avLockdownTimer) clearInterval(window._avLockdownTimer);
    window._avLockdownTimer=null;
    document.body.classList.remove('vector-lockdown-active','vector-lockdown-warning');
    const kills=state.rogueEvent?.kills||0;
    state.rogueEvent={active:false,errorAt:Date.now(),kills};
    state.rogueWarning={active:false};
    ensureRogueHud().classList.add('hidden');
    log('VECTOR LOCKDOWN safely cancelled because the event hit a runtime error. Exploration restored.');
    toast('Lockdown safely cancelled. Game restored.');
    try{ renderAll(); queueAutosave(); }catch(e){}
  }
  ...
  function render(){
    ...
    if(state.rogueWarning?.active && !state.rogueWarning?.arena?.freeRoam) drawLockdownSeals();
    drawObjectiveRoute();
  ...
  function drawPlayerLockdownHealthBar(){
    ...
    const compact=lockdownMobileCompact();
    const stats=combatStatBlock();
    const max=Math.max(1,state.rogueEvent?.playerMaxHp||stats.maxHp||state.player.maxHp||60);
  ...
  function drawLockdownActors(){
    ...
    for(const m of e.enemies||[]){
      ...
      const im=ensureImageCached(m.img || m.icon) || ensureImageCached(m.icon);
      if(im && im.complete && im.naturalWidth){
        ctx.save();
