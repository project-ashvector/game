...
> Main menu opens after the intro or if playback is blocked.`;
  ...
  function npcRouteBoardHtml(stage=currentStageKey()){
    ...
    const q=fermilatQuest(stage);
    const qPct=q ? Math.min(100, Math.floor((q.progress||0)/Math.max(1,q.target||1)*100)) : 0;
    const metalKey=`metallik:${stage}`;
    const metalClaimed=!!state?.npcRewards?.[metalKey];
  ...
  // v229: Route Prep gives players a clean before-boss readiness card.
  // v230: Portal Guide keeps travel rules readable after adding return portals.
  function portalGuideBoardHtml(stage=currentStageKey()){
    const def=stageDef(stage)||stageDef();
    const stageNum=Number(String(def.id||'').replace(/\D+/g,'')) || (Object.keys(STAGE_DEFS).indexOf(stage)+1);
    const hasReturn=stageNum>1 || stage!=='f001';
    const lockdown=!!state.vectorLockdown?.active || !!state.vectorLockdown?.warning;
    const bossDone=!!state.flags?.bossDefeated || !!state.stages?.[stage]?.complete;
    const exitStatus=bossDone?'Ready after boss/core clear':'Locked until boss/core objective is clear';
    const returnStatus=lockdown?'Locked during Vector Lockdown':hasReturn?'Ready before the boss or anytime normal routing is open':'No return portal on F-001';
    const rows=[
      ['S','Sync Terminal','Saves checkpoint and starts the route.'],
      ['N','Anomaly / Fight','Clear these to open the boss route.'],
      ['B','Boss Gate','Fight only when ready. Heal first.'],
      ['R','Return Portal',returnStatus],
      ['X','Exit Portal',exitStatus]
    ];
    return `<section class="fracture-card portal-guide-board"><div class="record-kicker">PORTAL + MAP GUIDE // ${safeHtml(def.id||stage)}</div><h3>Travel Rules</h3><p>Return portals are for backtracking. Exit portals are for pushing forward after the boss objective.</p><div class="portal-guide-grid">${rows.map(([sym,name,desc])=>`<div class="portal-guide-row"><b>${safeHtml(sym)}</b><span><strong>${safeHtml(name)}</strong>${safeHtml(desc)}</span></div>`).join('')}</div><p class="fineprint">Portals and NPC interactions lock only while Vector Lockdown is warning/active. When the event ends, normal route travel comes back.</p></section>`;
  }

  function routePrepBoardHtml(stage=currentStageKey()){
    const def=stageDef(stage)||stageDef();
    const stats=combatStatBlock();
    const hpPct=Math.floor(100*(state.player.hp||0)/Math.max(1,stats.maxHp||1));
    const epPct=Math.floor(100*(state.player.ep||0)/Math.max(1,stats.maxEp||state.player.maxEp||1));
    const med=Number(state.inventory?.['Med Patch']||0);
    const cell=Number(state.inventory?.['Vector Cell']||0);
    const required=requiredAnomaliesForStage(stage);
    const cleared=Math.min(required,state.flags?.anomaliesCleared||0);
    const bossReady=!!state.flags?.bossUnlocked;
    const bossDone=!!state.flags?.bossDefeated;
    const lockdown=!!state.vectorLockdown?.active || !!state.vectorLockdown?.warning;
    const checks=[
      ['HP Ready', hpPct>=55, `HP ${state.player.hp}/${stats.maxHp} (${hpPct}%)`],
      ['EP Ready', epPct>=35, `EP ${state.player.ep}/${stats.maxEp||state.player.maxEp} (${epPct}%)`],
      ['Med Patch', med>0, `${med} in bag`],
      ['Vector Cell', cell>0, `${cell} in bag`],
      ['Boss Route', bossReady || bossDone, bossDone?'Boss defeated':`${cleared}/${required} anomalies`],
      ['Lockdown Clear', !lockdown, lockdown?'Random event active/warning':'Normal route actions open']
    ];
    const good=checks.filter(r=>r[1]).length;
    const pct=Math.floor(100*good/Math.max(1,checks.length));
    const warning=lockdown?'Finish Vector Lockdown first. Portals and NPC actions are locked during the event.':(!bossReady&&!bossDone?'Clear the anomaly requirement before pushing the boss route.':'Ready to push, heal, backtrack, or extract depending on objective.');
    return `<section class="fracture-card route-prep-board"><div class="record-kicker">ROUTE PREP // ${safeHtml(def.id||stage)}</div><h3>Before You Push</h3><p>${safeHtml(warning)}</p><div class="statrow">Readiness ${good}/${checks.length}<div class="bar xp"><span style="width:${pct}%"></span></div></div><div class="route-prep-grid">${checks.map(([name,ok,val])=>`<div class="route-prep-check ${ok?'ready':'warn'}"><b>${ok?'✅':'⚠️'} ${safeHtml(name)}</b><span>${safeHtml(val)}</span></div>`).join('')}</div><div class="route-prep-actions"><button onclick="window.AV.useMedPatch()" ${med>0?'':'disabled'}>Use Med Patch</button><button onclick="window.AV.useVectorCell()" ${cell>0?'':'disabled'}>Use Vector Cell</button><button onclick="window.AV.showObjectivePing()">Ping Objective</button><button onclick="window.AV.openOverlay('inventoryOverlay')">Open Inventory</button></div><p class="fineprint">This panel is only a guide. It does not change boss requirements, rewards, saves, or NPC logic.</p></section>`;
  }

  function onboardingChecklistBoardHtml(stage=currentStageKey()){
    ensureContracts(); ensureSideQuests(); ensureNpcState();
    const required=requiredAnomaliesForStage(stage);
    const cleared=Math.min(required, state.flags?.anomaliesCleared || 0);
    const contract=activeContract(stage);
    const q=fermilatQuest(stage);
    const metalClaimed=!!state?.npcRewards?.[`metallik:${stage}`];
    const rows=[
      ['Terminal Synced', !!state.flags?.terminal, 'Touch the cyan terminal first so the map knows your route.'],
      ['Anomaly Route', cleared>=required, `Clear ${cleared}/${required} anomalies to open the boss path.`],
    ...
    const pct=Math.floor(100*done/Math.max(1,rows.length));
    return `<section class="fracture-card onboarding-checklist-board"><div class="record-kicker">NEW PLAYER ROUTE // LIVE CHECKLIST</div><h3>What To Do Next</h3><p>This keeps the level readable while testing. It updates automatically with your current level, NPCs, boss route, and rewards.</p><div class="statrow">Checklist ${done}/${rows.length}<div class="bar xp"><span style="width:${pct}%"></span></div></div><div class="onboarding-check-grid">${rows.map(([label,ok,hint])=>`<div class="onboarding-check-row ${ok?'done':'todo'}"><b>${ok?'✅':'⬜'} ${safeHtml(label)}</b><span>${safeHtml(hint)}</span></div>`).join('')}</div><div class="onboarding-actions"><button onclick="window.AV.showObjectivePing()">Ping Current Objective</button><button onclick="window.AV.resetTutorialTips()">Replay Tutorial Tips</button></div><p class="fineprint">Use this board when a level feels confusing. It does not change rewards or progression.</p></section>`;
  }
  ...
  const TRAINING_ITEM_ASSETS = {
    "Ash Pebble": "assets/items/custom/v212/ash_pebble.png",
    "Ash Ore": "assets/items/custom/v212/ash_ore.png",
    "Dense Ash Ore": "assets/items/custom/v212/dense_ash_ore.png",
    "Vector Crystal": "assets/items/custom/v212/vector_crystal.png",
    "Obsidian Core Ore": "assets/training/items/cryptomining/obsidian_core_ore.png",
    "Static Packet": "assets/items/custom/v212/static_packet.png",
    "Encrypted Data": "assets/training/items/datafishing/encrypted_data.png",
    "Ghost Log": "assets/items/custom/v212/ghost_log.png",
    "Blackbox File": "assets/items/custom/v212/blackbox_file.png",
    "Deep Archive Packet": "assets/training/items/datafishing/deep_archive_packet.png",
    "Wire Scrap": "assets/items/custom/v212/wire_scrap.png",
    "Circuit Scrap": "assets/items/custom/v212/circuit_scrap.png",
    "Logic Board": "assets/items/custom/v212/logic_board.png",
    "Quantum Relay": "assets/items/custom/v212/quantum_relay.png",
    "Null Module Part": "assets/training/items/codecraft/null_module_part.png",
    "Spore Sample": "assets/items/custom/v212/spore_sample.png",
    "Mutagen Sample": "assets/items/custom/v212/mutagen_sample.png",
    "Mutated Tissue": "assets/items/custom/v212/mutated_tissue.png",
    "Vector DNA": "assets/items/custom/v212/vector_dna.png",
    "Ash Genome Strand": "assets/training/items/forgenetics/ash_genome_strand.png",
    "Broken Token": "assets/items/custom/v212/broken_token.png",
    "Access Fragment": "assets/training/items/system_hacking/access_fragment.png",
    "Security Keybit": "assets/items/custom/v212/security_keybit.png",
    "Root Cipher": "assets/items/custom/v212/root_cipher.png",
  ...
  // v228: Boss Intel gives the Mission Briefing a clear pre-fight scouting card.
  function currentStageBossSlot(stage=currentStageKey()){
    const slots=ENCOUNTER_SLOTS[stage] || ENCOUNTER_SLOTS.f001 || {};
    const found=Object.entries(slots).find(([_,slot])=>slot && slot.type==='boss');
    if(!found) return null;
    const [xy,slot]=found;
    const [x,y]=xy.split(',').map(n=>Number(n));
    return {x,y,slot};
  }
  function bossIntelBoardHtml(stage=currentStageKey()){
    const def=stageDef(stage)||stageDef();
    const bossSlot=currentStageBossSlot(stage);
    let boss=null;
    if(bossSlot){
      const oldStage=state.currentStage;
      try{ state.currentStage=stage; boss=getEncounterDef('B', bossSlot.x, bossSlot.y); }catch(err){ boss=null; }
      state.currentStage=oldStage;
    }
    const required=requiredAnomaliesForStage(stage);
    const cleared=stage===currentStageKey()?Math.min(required, state.flags?.anomaliesCleared||0):0;
    const routeOpen=stage===currentStageKey()?!!state.flags?.bossUnlocked:false;
    const defeated=stage===currentStageKey()?!!state.flags?.bossDefeated:!!state.stages?.[stage]?.complete;
    const p=storyProfileForStage(stage);
    const loot=(boss?.loot||[]).concat(boss?.bossReward?[boss.bossReward]:[]);
    const lootHtml=(loot.length?loot:['Stage clear cache']).map(item=>`<span>${safeHtml(item)}</span>`).join('');
    const playerStats=combatStatBlock();
    const danger=boss?Math.max(1,Math.round((boss.atk||1)/Math.max(1,playerStats.def||1))):1;
    const ready=routeOpen || cleared>=required || defeated;
    const prep=[
      ready?'Boss route is open or ready. Heal before stepping into the boss tile.':`Clear ${required-cleared} more anomaly signature${required-cleared===1?'':'s'} to open the boss route.`,
      'Use Med Patch before the fight if HP is below half.',
      'Use Vector Cell if EP is low so skills stay available.',
      'Return portal can be used before the fight unless Vector Lockdown is active.'
    ];
    return `<section class="fracture-card boss-intel-board"><div class="record-kicker">BOSS INTEL // ${safeHtml(def.id)}</div><h3>${safeHtml(boss?.name || p.boss || 'Unknown Core Guardian')}</h3><p>${safeHtml(boss?.note || p.boss || 'Boss-class signature detected near the route core.')}</p><div class="boss-intel-layout"><div class="boss-intel-stat"><b>Route</b><span>${defeated?'Defeated':ready?'Open / Ready':'Locked'}</span></div><div class="boss-intel-stat"><b>Requirement</b><span>${cleared}/${required} anomalies</span></div><div class="boss-intel-stat"><b>Boss HP</b><span>${boss?.hp||'?'}</span></div><div class="boss-intel-stat"><b>Boss ATK</b><span>${boss?.atk||'?'} // danger ${danger}</span></div><div class="boss-intel-stat"><b>XP</b><span>${boss?.xp||'?'} Sync XP</span></div><div class="boss-intel-stat"><b>Credits</b><span>${boss?.credits||'?'}+</span></div></div><h4>Possible Drops</h4><div class="boss-drop-list">${lootHtml}</div><h4>Prep Notes</h4><div class="protocol-list compact-guide-list">${prep.map((line,i)=>`<div><b>${i+1}</b><span>${safeHtml(line)}</span></div>`).join('')}</div></section>`;
  }
  ...
  function renderMissionContractPanel(){
    ...
    panel.innerHTML = onboardingChecklistBoardHtml() + routePrepBoardHtml() + portalGuideBoardHtml() + playerGuideBoardHtml() + stageRewardPreviewBoardHtml() + bossIntelBoardHtml() + npcRouteBoardHtml() + contractHtml() + sideQuestHtml();
  ...
  const coreItemRegistry = [
    ...
    {id:'EQ-002', name:'Sewer Guard Vest', type:'Equipment', category:'Armor', slot:'Chest', rarity:'Common', stackSize:1, sellPrice:10, asset:'assets/items/custom/v212/sewer_guard_vest.png', status:'dedicated-item-art', levelReq:1, stats:{def:2,hp:8}, desc:'A cleaned-up vest from an unlucky guard. Smells like plot progression.'},
    {id:'EQ-003', name:'Rustcore Blade', type:'Weapon', category:'Weapons', slot:'Weapon', rarity:'Rare', stackSize:1, sellPrice:55, asset:'assets/items/custom/rustcore_blade.png', status:'dedicated-item-art', levelReq:3, stats:{atk:8,str:4,crit:0.03}, desc:'Crafted from anomaly metal. The edge hums like it knows your browser history.'},
    {id:'EQ-004', name:'Catalyst Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Epic', stackSize:1, sellPrice:90, asset:'assets/items/custom/catalyst_core.png', status:'dedicated-item-art', levelReq:5, stats:{atk:3,def:2,hp:20,ep:10}, desc:'A dangerous core module that boosts every important stat and probably voids warranties.'},
    {id:'EQ-005', name:'Toxic Monarch Relic', type:'Equipment', category:'Relic', slot:'Relic', rarity:'Legendary', stackSize:1, sellPrice:140, asset:'assets/items/custom/toxic_monarch_relic.png', status:'dedicated-item-art', levelReq:5, stats:{atk:5,str:5,def:3,hp:25,crit:0.05,xpBonus:0.05}, desc:'Boss-class relic. It whispers motivational insults before every fight.'},
    {id:'IT-008', name:'Burnt Alloy', type:'Material', category:'Crafting', slot:'Stack', rarity:'Uncommon', stackSize:999, sellPrice:6, asset:'assets/items/custom/burnt_alloy.png', status:'dedicated-item-art', desc:'Heat-scarred outpost metal used for Ash Wastes gear.'},
    {id:'IT-009', name:'Outpost Access Chip', type:'Key Item', category:'Access', slot:'Story', rarity:'Rare', stackSize:99, sellPrice:0, asset:'assets/items/custom/outpost_access_chip.png', status:'dedicated-item-art', desc:'Recovered access chip from the Ash Wastes Outpost route.'},
    {id:'EQ-006', name:'Ash-Plated Blade', type:'Weapon', category:'Weapons', slot:'Weapon', rarity:'Rare', stackSize:1, sellPrice:75, asset:'assets/items/custom/ash_plated_blade.png', status:'dedicated-item-art', levelReq:5, stats:{atk:11,str:6,crit:0.04}, desc:'A heavier ash-wrapped blade tuned for F-002 anomaly armor.'},
    {id:'EQ-007', name:'Wasteland Guard Helm', type:'Equipment', category:'Armor', slot:'Helm', rarity:'Rare', stackSize:1, sellPrice:65, asset:'assets/items/custom/v212/wasteland_guard_helm.png', status:'dedicated-item-art', levelReq:5, stats:{def:5,hp:18,ep:4}, desc:'Outpost helm with the HUD cracked but the skull protection still online.'},
    {id:'EQ-008', name:'Ashveil Mother Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:180, asset:'assets/items/custom/ashveil_mother_core.png', status:'dedicated-item-art', levelReq:7, stats:{atk:7,str:4,def:4,hp:36,ep:14,crit:0.04,xpBonus:0.06}, desc:'Boss-class core recovered from the imported Ashveil Spider Mother record. It pulses like an engine trying to insult you.'},
    {id:'EQ-009', name:'Duskwither Wraith Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:220, asset:'assets/items/custom/duskwither_wraith_core.png', status:'dedicated-item-art', levelReq:12, stats:{atk:9,str:6,def:6,hp:46,ep:18,crit:0.06,xpBonus:0.08}, desc:'Boss-class core recovered from the imported Duskwither Shade Wraith record. It hums like a haunted GPU and boosts late-stage training.'},
    {id:'EQ-010', name:'Transit Nexus Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:260, asset:'assets/items/custom/transit_nexus_core.png', status:'dedicated-item-art', levelReq:15, stats:{atk:10,str:5,def:7,hp:54,ep:22,crit:0.06,xpBonus:0.08}, desc:'Boss-class rail core recovered from the Transit Nexus Horror. It sparks like a subway map having a panic attack.'},
    {id:'EQ-011', name:'Prism Wound Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:300, asset:'assets/items/custom/prism_wound_core.png', status:'dedicated-item-art', levelReq:20, stats:{atk:12,str:6,def:8,hp:62,ep:28,crit:0.07,xpBonus:0.09}, desc:'Boss-class prism core recovered from the Glass Storm Lab. It bends damage, light, and bad choices.'},
    {id:'EQ-012', name:'Vector Heart Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:360, asset:'assets/items/custom/vector_heart_core.png', status:'dedicated-item-art', levelReq:25, stats:{atk:15,str:8,def:10,hp:76,ep:34,crit:0.08,xpBonus:0.10}, desc:'Mechanical heart core pulled from the Vector Heart Tyrant. It still beats in blue-vector pulses.'},
    {id:'EQ-013', name:'Cinderline Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:420, asset:'assets/items/custom/cinderline_core.png', status:'dedicated-item-art', levelReq:30, stats:{atk:18,str:10,def:12,hp:90,ep:38,crit:0.085,xpBonus:0.105}, desc:'Locomotive furnace core from Cinder Express Yard. Hot enough to make the inventory nervous.'},
    {id:'EQ-014', name:'Drowned Archive Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:500, asset:'assets/items/custom/drowned_archive_core.png', status:'dedicated-item-art', levelReq:35, stats:{atk:20,str:11,def:15,hp:104,ep:46,crit:0.09,xpBonus:0.11}, desc:'Waterlogged data-core relic recovered from the Flooded Data Vault. It leaks corrupted archive light.'},
    {id:'EQ-015', name:'Harvest Alloy Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:580, asset:'assets/items/custom/harvest_alloy_core.png', status:'dedicated-item-art', levelReq:40, stats:{atk:23,str:13,def:17,hp:118,ep:52,crit:0.095,xpBonus:0.115}, desc:'Rust-orchard reactor core shaped by metal fruit and bad farming machinery.'},
    {id:'EQ-016', name:'Parallax Lens Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:680, asset:'assets/items/custom/parallax_lens_core.png', status:'dedicated-item-art', levelReq:45, stats:{atk:26,str:15,def:19,hp:132,ep:60,crit:0.10,xpBonus:0.12}, desc:'Blacksite observatory lens core that stares back from every angle.'},
    {id:'EQ-017', name:'Basilica Wyrm Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Legendary', stackSize:1, sellPrice:780, asset:'assets/items/custom/basilica_wyrm_core.png', status:'dedicated-item-art', levelReq:50, stats:{atk:30,str:17,def:22,hp:148,ep:68,crit:0.105,xpBonus:0.125}, desc:'Frozen wyrm heart sealed inside a bone reliquary from Cryo Basilica.'},
    {id:'EQ-018', name:'Ash Crown Core', type:'Equipment', category:'Core', slot:'Core', rarity:'Singularity', stackSize:1, sellPrice:950, asset:'assets/items/custom/ash_crown_core.png', status:'dedicated-item-art', levelReq:55, stats:{atk:36,str:20,def:26,hp:170,ep:80,crit:0.12,xpBonus:0.14}, desc:'Ash-crowned final-route reactor core. It hums like royalty with a static problem.'},
    {id:'IT-026', name:'Operator Shard: Vexa', type:'Shard', category:'Operator', slot:'Recruitment', rarity:'Legendary', stackSize:999, sellPrice:75, asset:'assets/items/custom/operator_shard_vexa.png', status:'dedicated-item-art', desc:'A synchronization shard keyed to Operator Vexa.'},
    {id:'IT-027', name:'Projectile Coil', type:'Material', category:'Lockdown', slot:'Stack', rarity:'Rare', stackSize:999, sellPrice:18, asset:'assets/items/custom/projectile_coil.png', status:'dedicated-item-art', desc:'Compact railgun coil recovered from Vector Lockdown events.'},
    {id:'IT-028', name:'Ash Ammo Cache', type:'Material', category:'Lockdown', slot:'Stack', rarity:'Uncommon', stackSize:999, sellPrice:12, asset:'assets/items/custom/ash_ammo_cache.png', status:'dedicated-item-art', desc:'Ash-covered ammo cache recovered from Vector Lockdown events.'},
    {id:'IT-029', name:'Vector Shard Cache', type:'Material', category:'Lockdown', slot:'Stack', rarity:'Epic', stackSize:999, sellPrice:35, asset:'assets/items/custom/vector_shard_cache.png', status:'dedicated-item-art', desc:'Locked cache of glowing vector shards recovered from Vector Lockdown events.'},
    {id:'IT-020', name:'Ash Ore', type:'Material', category:'Mining', slot:'Stack', rarity:'Common', stackSize:999, sellPrice:3, asset:'assets/items/custom/v212/ash_ore.png', status:'dedicated-item-art', desc:'Ore chipped from ash-veined salvage nodes. Used for future smithing and upgrade loops.'},
    ...
    {id:'IT-022', name:'Circuit Scrap', type:'Material', category:'Codecraft', slot:'Stack', rarity:'Common', stackSize:999, sellPrice:3, asset:'assets/items/custom/v212/circuit_scrap.png', status:'dedicated-item-art', desc:'Circuit fragments used for future codecraft and gear assembly.'},
    {id:'IT-023', name:'Mutagen Sample', type:'Material', category:'Forgenetics', slot:'Stack', rarity:'Rare', stackSize:999, sellPrice:8, asset:'assets/items/custom/v212/mutagen_sample.png', status:'dedicated-item-art', desc:'Unstable bio sample harvested from corrupted growths.'},
...
,
  ...
  function buyCharacterNow(id){
    ...
    const progress=operatorUnlockProgress(id);
    if(progress.unlocked){ toast(`${op.displayName} is already unlocked.`); renderCharacterMenuDb(id); return true; }
    const url=op.buyUrl || op.storeUrl || op.purchaseUrl;
    if(url){ window.open(url, '_blank', 'noopener'); log(`Opened purchase link for ${op.displayName}.`); return true; }
    toast(`Buy Now for ${op.displayName} is a store placeholder for this build.`);
    log(`Buy Now selected for ${op.displayName}. Store link not configured yet.`);
  ...
  function scheduleNextVectorLockdown(reason='cooldown'){
    const now=Date.now();
    const min=90000, max=210000;
    const gap=min+Math.floor(Math.random()*(max-min));
    state.rogueLastAt=now;
    state.rogueNextAt=now+gap;
    if(reason) log(`Next Vector Lockdown window in ${Math.ceil(gap/1000)}s // ${reason}.`);
    return state.rogueNextAt;
  }
  function ensureLockdownRewardModal(){
    let modal=$('vectorLockdownRewardModal');
    if(!modal){
      modal=document.createElement('div');
      modal.id='vectorLockdownRewardModal';
      modal.className='vector-lockdown-reward-modal hidden';
  ...
  function restoreLockdownRewardModal(){
    if(state?.rogueHudMode==='reward' && state?.lockdownRewardPopup?.html){
      const modal=$('vectorLockdownRewardModal');
      if(!modal || modal.classList.contains('hidden') || modal.style.display==='none' || !modal.innerHTML){
        showLockdownRewardModal(state.lockdownRewardPopup.html);
      }
    }
  }
  ...
  function maybeTriggerVectorLockdown(){
    if(isGameplayPaused()) return;
    if(vectorLockdownBlocked()) return;
    const now=Date.now();
    if(!Number.isFinite(Number(state.rogueNextAt))) scheduleNextVectorLockdown('initial randomized timer');
    if(now < Number(state.rogueNextAt||0)) return;
  ...
  function lockdownArenaBounds(cx=state.player.x, cy=state.player.y){
    ...
    return {minX:0,minY:0,maxX:Math.max(0,w-1),maxY:Math.max(0,h-1),freeRoam:true,center:{x:cx,y:cy}};
  }
  function pointInLockdownArena(x,y){
    return Number.isFinite(x) && Number.isFinite(y) && x>=0 && y>=0 && x<mapWidth() && y<mapHeight();
  }
  function lockdownNormalInteractionsLocked(){ return !!state.rogueEvent?.active; }
  function lockdownInteractionToast(label='interaction'){
    toast(`VECTOR LOCKDOWN: ${label} paused until the surge ends.`);
    showXpFloat('LOCKDOWN ACTIVE','locked');
    return true;
  }
  function lockdownTier(){
    const n=Number((currentStageKey()||'f001').replace(/[^0-9]/g,''))||1;
    if(n>=10) return {name:'Red Alert', reward:4, hp:1.55, speed:1.22, damage:1.18};
    if(n>=7) return {name:'Ash Collapse', reward:3, hp:1.35, speed:1.14, damage:1.12};
    if(n>=4) return {name:'Vector Surge', reward:2, hp:1.18, speed:1.08, damage:1.06};
    return {name:'Minor Lockdown', reward:1, hp:1, speed:1, damage:1};
  }
  function lockdownOperatorBonus(){
    ...
    else if(id==='brakk'){ bonus.label='Brakk heavy rounds'; bonus.damage=7; bonus.fireRate=1.08; }
  ...
  function updateVectorLockdownWarningHud(){
    ...
    if(!w?.active){ hud.classList.add('hidden'); return; }
  ...
  function tickVectorLockdownWarning(){
    ...
    if(!w?.active){ cancelVectorLockdownWarning('Vector warning ended.'); return; }
    if(battle || storyActive){ cancelVectorLockdownWarning('Vector warning paused by story/combat.'); return; }
    updateVectorLockdownWarningHud();
    if(Date.now()>=(w.endsAt||0)) startVectorLockdown(w);
    render();
  }
  function startVectorLockdown(warning=null){
    ...
    window._avLockdownTimer=setInterval(tickVectorLockdown,60);
    renderAll();
  }
  function lockdownSpawnPointAwayFromPlayer(){
    const px=state.player.x+.5, py=state.player.y+.5;
    const candidates=[];
    for(let attempt=0;attempt<120;attempt++){
      let x,y;
      const edge=Math.floor(Math.random()*4);
      if(edge===0){ x=Math.floor(camera.x/TILE)-1; y=Math.floor((camera.y+Math.random()*VIEW_H)/TILE); }
      else if(edge===1){ x=Math.floor((camera.x+VIEW_W)/TILE)+1; y=Math.floor((camera.y+Math.random()*VIEW_H)/TILE); }
      else if(edge===2){ x=Math.floor((camera.x+Math.random()*VIEW_W)/TILE); y=Math.floor(camera.y/TILE)-1; }
      else { x=Math.floor((camera.x+Math.random()*VIEW_W)/TILE); y=Math.floor((camera.y+VIEW_H)/TILE)+1; }
      x=Math.max(1,Math.min(mapWidth()-2,x)); y=Math.max(1,Math.min(mapHeight()-2,y));
      if(canStandAt(x,y) && Math.hypot(x-px,y-py)>4) candidates.push({x:x+.5,y:y+.5});
    }
    if(candidates.length) return candidates[Math.floor(Math.random()*candidates.length)];
    for(let attempt=0;attempt<200;attempt++){ const x=1+Math.floor(Math.random()*Math.max(1,mapWidth()-2)); const y=1+Math.floor(Math.random()*Math.max(1,mapHeight()-2)); if(canStandAt(x,y) && Math.hypot(x-px,y-py)>4) return {x:x+.5,y:y+.5}; }
    return {x:state.player.x+2.5,y:state.player.y+2.5};
  }
  function spawnLockdownEnemy(e){
    if(!e?.active || (e.enemies||[]).length >= (e.allowedHostiles||e.maxHostiles||100)) return false;
    const roster=importedAnomalyRoster.length ? importedAnomalyRoster : [{name:'Vector Hostile',battle:''}];
    const stageN=Number((currentStageKey()||'f001').replace(/[^0-9]/g,''))||1;
    ...
    e.enemies.push({
      ...
      size:type==='tank'?48:(type==='fast'?34:40), type, touchAt:0
    });
    return true;
  }
  ...
  function fireLockdownVolley(e){
    ...
    for(let i=0;i<count;i++){
      const spread=(i-(count-1)/2)*0.15; const angle=base+spread+(Math.random()-.5)*0.035;
      const asset=lockdownProjectileAsset((e.kills||0)+i+(e.upgradeHistory?.length||0));
      const speed=(e.projectileSpeed||8.2)*(asset.path.includes('needle')?1.18:1);
  ...
  function abortVectorLockdown(reason='runtime guard'){
    ...
    state.rogueHudMode=null;
    scheduleNextVectorLockdown('event aborted');
    hideLockdownRewardModal();
    ensureRogueHud().classList.add('hidden');
    ensureLockdownBuffDock().classList.add('hidden');
  ...
  function tickVectorLockdown(){
    try{
      ...
      if(isGameplayPaused()){ e.lastFrameAt=Date.now(); updateVectorLockdownHud(); return; }
  ...
  function updateLockdownActors(e,dt){
    ...
    for(const m of e.enemies){
      ...
      for(const o of e.enemies){
        if(o===m) continue;
        const odx=m.x-o.x, ody=m.y-o.y, od=Math.hypot(odx,ody);
    ...
    for(const p of e.projectiles){
      const oldX=p.x, oldY=p.y; p.x+=p.vx*(dt/1000); p.y+=p.vy*(dt/1000); p.life-=dt;
      if(p.trail){ p.trail.push({x:p.x,y:p.y}); if(p.trail.length>6) p.trail.shift(); }
      let keep=p.life>0 && pointInLockdownArena(Math.floor(p.x),Math.floor(p.y));
      for(const m of e.enemies){
        if(m.hp<=0) continue;
      ...
      if(keep) liveProjectiles.push(p);
    }
    e.projectiles=liveProjectiles.slice(-150);
    e.enemies=e.enemies.filter(m=>m.hp>0).slice(0,e.maxHostiles||100);
    e.floaters=(e.floaters||[]).filter(f=>now-(f.born||now)<(f.life||720));
  }


  function addLockdownFloat(e,x,y,text,type='hit'){
    if(!e) return;
    e.floaters ||= [];
    e.floaters.push({x,y,text:String(text),type,born:Date.now(),life:type==='kill'?920:720});
    if(e.floaters.length>80) e.floaters=e.floaters.slice(-80);
  }

  function updateLockdownHpHud(){
    const e=state.rogueEvent||{};
    const max=Math.max(1,e.playerMaxHp||combatStatBlock().maxHp||state.player.maxHp||60);
    const hp=Math.max(0,Math.ceil(state.player.hp||0));
    const pct=Math.max(0,Math.min(100,(hp/max)*100));
    const txt=$('lockdownHpText'); if(txt) txt.textContent=`${hp}/${max}`;
  ...
  function failVectorLockdown(killer=null){
    ...
    scheduleNextVectorLockdown('failed run');
  ...
  function drawLockdownSeals(){
    ...
    const compact=lockdownMobileCompact();
    const label=compact?(isWarning?'!':''):(isWarning?'WARN':'');
  ...
  function drawLockdownEdgeHue(){
    if(!state.rogueEvent?.active) return;
    const pulse=.18+.10*Math.sin(Date.now()*.012);
    ctx.save();
    ctx.setTransform(1,0,0,1,0,0);
    const grd=ctx.createRadialGradient(VIEW_W/2,VIEW_H/2,Math.min(VIEW_W,VIEW_H)*.28,VIEW_W/2,VIEW_H/2,Math.max(VIEW_W,VIEW_H)*.72);
    grd.addColorStop(0,'rgba(255,48,72,0)');
    grd.addColorStop(.62,`rgba(255,48,72,${pulse*.18})`);
    grd.addColorStop(1,`rgba(255,48,72,${pulse})`);
    ctx.fillStyle=grd;
    ctx.fillRect(0,0,VIEW_W,VIEW_H);
    ctx.strokeStyle=`rgba(255,48,72,${pulse+.10})`;
    ctx.lineWidth=lockdownMobileCompact()?6:8;
  ...
  function lockdownNameTag(name){
    return String(name||'AN').replace(/[^a-z0-9 ]/gi,'').trim().split(/\s+/).slice(0,2).map(w=>w[0]).join('').toUpperCase() || 'AN';
  }
  function drawLockdownActors(){
    ...
    for(const m of e.enemies||[]){
      ...
      if(im && im.complete && im.naturalWidth){
        ...
        ctx.shadowBlur=compact?10:8;
        const oldSmooth=ctx.imageSmoothingEnabled;
        ctx.imageSmoothingEnabled=true;
        ctx.drawImage(im, x-drawW/2, y-drawH/2+bob, drawW, drawH);
        ctx.imageSmoothingEnabled=oldSmooth;
        ctx.restore();
        if(Date.now()-(m.hitFlashAt||0)<130){
          ctx.save(); ctx.globalCompositeOperation='screen'; ctx.fillStyle='rgba(255,255,255,.55)';
          ctx.beginPath(); ctx.ellipse(x,y+bob,drawW*.42,drawH*.42,0,0,Math.PI*2); ctx.fill(); ctx.restore();
        }
      } else {
        // Strong fallback so the event never becomes invisible if an asset path is missing.
        const pulse=.72+.18*Math.sin(Date.now()*.009+m.phase);
        const grd=ctx.createRadialGradient(x,y+bob,3,x,y+bob,drawW*.48);
        grd.addColorStop(0,`rgba(255,238,190,${pulse})`);
        grd.addColorStop(.45,'rgba(255,88,72,.94)');
        grd.addColorStop(1,'rgba(120,18,28,.62)');
        ctx.fillStyle=grd;
        ctx.beginPath();
        ctx.moveTo(x,y-drawH*.45+bob);
        ctx.lineTo(x+drawW*.38,y-drawH*.08+bob);
        ctx.lineTo(x+drawW*.26,y+drawH*.38+bob);
        ctx.lineTo(x,y+drawH*.24+bob);
        ctx.lineTo(x-drawW*.26,y+drawH*.38+bob);
        ctx.lineTo(x-drawW*.38,y-drawH*.08+bob);
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle='rgba(255,255,255,.38)'; ctx.lineWidth=2; ctx.stroke();
        ctx.fillStyle='rgba(255,255,255,.95)'; ctx.font=compact?'900 10px monospace':'900 9px monospace'; ctx.textAlign='center'; ctx.fillText(lockdownNameTag(m.name),x,y+3+bob);
      }
      ctx.fillStyle='rgba(0,0,0,.78)'; ctx.fillRect(x-drawW*.42,y-drawH*.68,drawW*.84,6);
      ctx.fillStyle='rgba(255,48,72,.95)'; ctx.fillRect(x-drawW*.42,y-drawH*.68,drawW*.84*hpPct,6);
      ctx.strokeStyle='rgba(255,255,255,.28)'; ctx.strokeRect(x-drawW*.42-.5,y-drawH*.68-.5,drawW*.84+1,7);
    }
  ...
  function drawLockdownProjectiles(){
    ...
    for(const p of e.projectiles||[]){
      ...
      ctx.shadowBlur=16;
      if(im && im.complete && im.naturalWidth){
        const size=(p.size||30) * (lockdownMobileCompact()?1.16:1);
        const oldSmooth=ctx.imageSmoothingEnabled;
        ctx.imageSmoothingEnabled=true;
  ...
  function onboardingMiniStatusHtml(){
  ...
  function routeRecordStatus(key){
    ensureSaveShape();
    const def=stageDef(key);
    const rec=state.stages?.[key] || {};
    const unlocked=!!rec.unlocked || key==='f001' || playerMeetsStageRequirement(key);
    const complete=!!rec.complete;
    const bossKills=(state.bossKills||{})[key] || 0;
    const enemyKills=(state.enemyKills||{})[key] || 0;
    const metalClaimed=!!(state.npcRewards||{})[`metallik:${key}`];
    const label=complete?'Complete':unlocked?'Available':'Locked';
    const cls=complete?'complete':unlocked?'active':'locked';
    return {def, unlocked, complete, bossKills, enemyKills, metalClaimed, label, cls};
  }
  function routeRecordsBoardHtml(){
    const rows=Object.keys(STAGE_DEFS).map(key=>{
      const r=routeRecordStatus(key);
      const req=r.def.levelReq||1;
      const cache=r.metalClaimed?'Cache claimed':'Cache ready';
      const boss=r.bossKills>0?`${r.bossKills} core${r.bossKills===1?'':'s'}`:'No boss core';
      const clear=r.complete?'Clear reward claimed':r.unlocked?'Clear reward pending':'Reach requirement first';
      return `<div class="route-record-row ${r.cls}"><div><b>${safeHtml(r.def.id)} ${safeHtml(r.def.title)}</b><span>${safeHtml(r.label)} // Req Lv ${req}</span></div><div><span>${safeHtml(boss)}</span><span>${r.enemyKills} anomaly kills</span><span>${safeHtml(cache)}</span><span>${safeHtml(clear)}</span></div></div>`;
    }).join('');
    const complete=Object.keys(STAGE_DEFS).filter(k=>state.stages?.[k]?.complete).length;
    const total=Object.keys(STAGE_DEFS).length;
    const bossTotal=Object.values(state.bossKills||{}).reduce((a,b)=>a+(Number(b)||0),0);
    const current=stageDef();
    return `<div class="record-kicker">ROUTE RECORDS // FRACTURE ARCHIVE</div><h3>World Progress</h3><p>${complete}/${total} fractures cleared // ${bossTotal} boss cores recovered // Current: ${safeHtml(current.id)} ${safeHtml(current.title)}</p><div class="route-record-list">${rows}</div>`;
  }
  function renderRoutePrepPanel(){
    const grid=document.querySelector('#missionOverlay .fracture-grid');
    if(!grid) return;
    let panel=$('routePrepBoard');
    if(!panel){ panel=document.createElement('section'); panel.id='routePrepBoard'; panel.className='fracture-card route-prep-board'; grid.appendChild(panel); }
    panel.outerHTML=routePrepBoardHtml();
  }

  function renderRouteRecordsPanel(){
    const grid=document.querySelector('#missionOverlay .fracture-grid');
    if(!grid) return;
    let panel=$('routeRecordsBoard');
    if(!panel){ panel=document.createElement('section'); panel.id='routeRecordsBoard'; panel.className='fracture-card route-records-board'; grid.appendChild(panel); }
  ...
  function closeOverlays(){
    radioMode=false;
    setGameplayPaused(false,'overlay close');
    document.querySelectorAll('.overlay').forEach(o=>{ o.classList.add('hidden'); o.style.display=''; });
    document.body.classList.remove('menu-protocol-open');
  ...
  // v217: true pause helpers. The pause menu now freezes timed gameplay systems instead of only opening a UI layer.
  let gameplayPauseStartedAt = 0;
  function gameplayPauseMenuVisible(){
    const panel=$('mobilePauseMenu');
    return !!(panel && !panel.classList.contains('hidden'));
  }
  function isGameplayPaused(){
    return !!(document.body.classList.contains('game-paused') || gameplayPauseMenuVisible());
  }
  function shiftTimedGameStateAfterPause(delta=0){
    delta = Math.max(0, Number(delta)||0);
    if(!delta || !state) return;
    if(state.rogueNextAt) state.rogueNextAt += delta;
    if(state.rogueWarning?.active){
      state.rogueWarning.startedAt = (state.rogueWarning.startedAt||Date.now()) + delta;
      state.rogueWarning.endsAt = (state.rogueWarning.endsAt||Date.now()) + delta;
    }
    const e=state.rogueEvent;
    if(e?.active){
      ['startedAt','lastFrameAt','nextUpgradeAt','nextSpawnAt','nextShotAt','nextOrbitAt'].forEach(k=>{ if(e[k]) e[k]+=delta; });
      (e.projectiles||[]).forEach(p=>{ if(p.spawnedAt) p.spawnedAt+=delta; });
      (e.enemies||[]).forEach(m=>{ if(m.spawnedAt) m.spawnedAt+=delta; if(m.hitAt) m.hitAt+=delta; });
    }
  ...
  function setGameplayPaused(paused, source='pause menu'){
    ...
    if(document.body.classList.contains('game-paused')){
      const delta = gameplayPauseStartedAt ? now - gameplayPauseStartedAt : 0;
      shiftTimedGameStateAfterPause(delta);
  ...
  function hideMobilePauseMenu(){
    ...
    setGameplayPaused(false,'pause menu');
    if(gameStarted && !$('app').classList.contains('hidden')){
      uiState.mode='game';
      try{ canvas.focus({preventScroll:true}); }catch(err){}
    }
  }
  ...
  function showMobilePauseMenu(){
    ...
    if(!panel){
      ...
      document.body.appendChild(panel);
      $('mobilePauseClose').onclick=hideMobilePauseMenu;
      panel.addEventListener('click',e=>{
        ...
        panel.classList.add('hidden'); panel.style.display=''; openOverlay(target);
      });
    }
    setGameplayPaused(true,'pause menu');
    panel.classList.remove('hidden');
  ...
  const ControllerManager = {
    ...
    backFromTopMenu(){
      ...
      if(pause && !pause.classList.contains('hidden')){ hideMobilePauseMenu(); return true; }
    ...
    handle(pad){
      ...
      if(pause && !pause.classList.contains('hidden')){
        if(start){ hideMobilePauseMenu(); return; }
  ...
  function bind(){
    ...
    document.addEventListener('pointerdown', e=>{
      ...
      if((e.key==='Escape' || String(e.key||'').toLowerCase()==='p') && gameIsOpen && !overlayOpen){ e.preventDefault(); showMobilePauseMenu(); return; }
  ...
  // V283: Phone overlay guard. Some phone/menu buttons can open overlays through older
  // paths, so keep the body menu class synced with actual visible overlays. This lets
  // CSS reliably hide gameplay HUD/touch controls and give the menu the whole screen.
  function syncPhoneOverlayClass(){
    try{
      const visible = Array.from(document.querySelectorAll('.overlay')).some(o=>{
        if(!o || o.classList.contains('hidden')) return false;
        const st = getComputedStyle(o);
        return st.display !== 'none' && st.visibility !== 'hidden';
      });
      document.body.classList.toggle('menu-protocol-open', !!visible);
    }catch(err){}
  }
  try{
    const mo = new MutationObserver(syncPhoneOverlayClass);
    document.querySelectorAll('.overlay').forEach(o=>mo.observe(o,{attributes:true,attributeFilter:['class','style']}));
    setInterval(syncPhoneOverlayClass, 500);
    window.addEventListener('resize', syncPhoneOverlayClass, {passive:true});
