...
,
  ...
  function updateLockdownActors(e,dt){ const px=state.player.x+.5, py=state.player.y+.5; const now=Date.now(); for(const m of e.enemies){ const dx=px-m.x, dy=py-m.y, d=Math.max(.05, Math.hypot(dx,dy)); m.x += (dx/d)*m.speed*dt; m.y += (dy/d)*m.speed*dt; if(e.arena){ m.x=Math.max(e.arena.minX+.25, Math.min(e.arena.maxX+.75, m.x)); m.y=Math.max(e.arena.minY+.25, Math.min(e.arena.maxY+.75, m.y)); } if(d<.58 && now>(m.touchAt||0)){ const hit=Math.max(2, Math.floor(3+Math.random()*4)); state.player.hp=Math.max(0,(state.player.hp||0)-hit); e.damageTaken=(e.damageTaken||0)+hit; m.touchAt=now+1050; showXpFloat(`-${hit} HP`,'locked'); if(state.player.hp<=0){ failVectorLockdown(m); return; } } } const liveProjectiles=[]; for(const p of e.projectiles){ const oldX=p.x, oldY=p.y; p.x+=p.vx*(dt/1000); p.y+=p.vy*(dt/1000); p.life-=dt; if(p.trail){ p.trail.push({x:p.x,y:p.y}); if(p.trail.length>5) p.trail.shift(); } let keep=p.life>0 && pointInLockdownArena(Math.floor(p.x),Math.floor(p.y)); for(const m of e.enemies){ if(m.hp<=0) continue; if(lockdownSegmentDistance(m.x,m.y,oldX,oldY,p.x,p.y)<.68){ m.hp-=p.damage; showXpFloat(`${p.damage}`,'xp'); if(p.pierce>0){ p.pierce--; } else { keep=false; } if(m.hp<=0){ e.kills++; if(Math.random()<.14) e.rewards++; showXpFloat('KILL','good'); } if(!keep) break; } } if(keep) liveProjectiles.push(p); } e.projectiles=liveProjectiles.slice(-120); e.enemies=e.enemies.filter(m=>m.hp>0).slice(0,e.maxHostiles||15); }
  function updateVectorLockdownHud(){ const e=state.rogueEvent; const hud=ensureRogueHud(); if(!e?.active){ hud.classList.add('hidden'); return; } updateLockdownHpHud(); if($('lockdownKicker')) $('lockdownKicker').textContent='VECTOR LOCKDOWN'; const left=Math.max(0,Math.ceil((e.duration-(Date.now()-e.startedAt))/1000)); $('lockdownTimer').textContent=`${left}s`; $('lockdownText').textContent=`EXITS SEALED // Auto-fire using ${e.starterWeapon}. Max ${e.maxHostiles||15} real monsters at once.`; if($('lockdownStats')) $('lockdownStats').innerHTML=`<span>Shots x${e.projectileCount}</span><span>DMG ${e.projectileDamage}</span><span>Rate ${Math.round(1000/e.fireRate*10)/10}/s</span><span>Kills ${e.kills}</span><span>Hostiles ${e.enemies.length}/${e.maxHostiles||15}</span>`; $('lockdownUpgrades').innerHTML=(e.upgrades||[]).slice(-8).map(u=>`<span>${safeHtml(u)}</span>`).join('') || '<span>First projectile upgrade incoming...</span>'; }
  ...
  function drawPlayerLockdownHealthBar(){
    ...
    ctx.fillStyle='rgba(255,255,255,.92)'; ctx.font='700 7px monospace'; ctx.textAlign='center'; ctx.fillText(`${Math.ceil(state.player.hp||0)}/${max}`,x,y-2);
    ctx.restore();
  }
